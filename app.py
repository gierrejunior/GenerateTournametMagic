...
if __name__ == "__main__":
    ...
    if not st.session_state.get("players_last_duel"):
        st.session_state["players_last_duel"] = []
        ...
        if len(st.session_state.get("players")) >= 2:  # type: ignore
            ...
            if is_random_decks:
                ...
                if st.session_state.get("players_and_decks"):
                    ...
                    all_decks = [
                        deck
                        for player_decks in st.session_state[
                            "players_and_decks"
                        ].values()
                    ...
                    if len(set(all_decks)) >= 2:
                        repeat_deck = st.radio("Pode repetir o deck?", ("Sim", "Não"))
                        can_repeat_deck = True if repeat_deck == "Sim" else False

            if not is_random_decks:
                if st.checkbox("Adicionar jogadores do ultimo duelo"):
                    ...
                    if len(players_last_duel) > 2:
                        ...
                        st.session_state["players_last_duel"] = (
                            players_last_duel  # Atualiza o valor no session_state
                        )
                ...
                if is_deactivate_deck:
                    ...
                    if len(active_decks) > 1:
                        ...
                        for deck, is_active in player_decks.items():
                            is_checked = (
                                not is_active
                            )  # O deck está marcado se estiver desativado
            ...
            if st.session_state.get("duel"):
                ...
                if isinstance(duel, dict):
                    ...
                    for i in range(1, 3):
                        ...
                        player_decks = st.session_state.players_and_decks.get(player)
                ...
                if col3.button("Salvar Duelo"):
                    if isinstance(duel, list):
                        players_last_duel = [duel[0], duel[1]]
                        ...
                        players_last_duel = [duel.get("player_1"), duel.get("player_2")]
                        ...
                        if players_last_duel:
                            ...
                            st.session_state["players_last_duel"] = players_last_duel
                st.write(st.session_state["players_last_duel"])
            ...
            st.warning("Você precisa ter mais de um jogador para gerar um duelo.")
